...
/// \brief Contains helper macros and implementation details of ddboost::typeindex::ctti_type_index.
...
#   include <boost/preprocessor/facilities/expand.hpp>
    ...
    // sizeof("const char *__cdecl ddboost::detail::ctti<") - 1, sizeof(">::n(void)") - 1
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(40, 10, false, "")
#elif defined(_MSC_VER) && !defined(__clang__) && !defined (BOOST_NO_CXX11_NOEXCEPT)
    // sizeof("const char *__cdecl ddboost::detail::ctti<") - 1, sizeof(">::n(void) noexcept") - 1
    ...
    // sizeof("static const char *ddboost::detail::ctti<") - 1, sizeof("]") - 1, true, "???????????>::n() [T = int"
    ...
    // sizeof("static const char *ddboost::detail::ctti<") - 1, sizeof(">::n()") - 1
    // note: checked on 3.0
    ...
    // sizeof("static const char *ddboost::detail::ctti<") - 1, sizeof("]") - 1, true, "int>::n() [T = int"
    // note: checked on 3.1, 3.4
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(39, 1, true, "T = ")
#elif defined(__EDG__) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static cha ddboost::detail::ctti<T>::s() [with I = 40U, T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(58, 1, false, "")
#elif defined(__EDG__) && defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static const char *ddboost::detail::ctti<T>::n() [with T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
    ...
    // sizeof("static constexpr char ddboost::detail::ctti<T>::s() [with unsigned int I = 0u; T = ") - 1, sizeof("]") - 1
    BOOST_TYPE_INDEX_REGISTER_CTTI_PARSING_PARAMS(81, 1, false, "")
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char ddboost::detail::ctti<T>::s() [with unsigned int I = 0; T = ") - 1, sizeof("]") - 1
...
namespace ddboost { namespace typeindex { namespace detail {
    ...
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining_runtime(const char* begin, ddboost::false_type) BOOST_NOEXCEPT {
        return begin;
    }
    ...
    BOOST_CXX14_CONSTEXPR inline int constexpr_strcmp(const char *v1, const char *v2) BOOST_NOEXCEPT {
        ...
        if (ddboost::typeindex::detail::is_constant_string(v1) && ddboost::typeindex::detail::is_constant_string(v2))
            return ddboost::typeindex::detail::constexpr_strcmp_loop(v1, v2);
    ...
    template <unsigned int ArrayLength>
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining_runtime(const char* begin, ddboost::true_type) BOOST_NOEXCEPT {
    ...
    BOOST_CXX14_CONSTEXPR inline const char* skip_begining(const char* begin) BOOST_NOEXCEPT {
        ...
        return skip_begining_runtime<ArrayLength - ctti_skip_size_at_begin>(
            ...
            ddboost::integral_constant<bool, ctti_skip_more_at_runtime>()
        );
    }
...
namespace ddboost { namespace detail {
...
struct ctti {
    ...
    constexpr static const char* impl(::ddboost::typeindex::detail::index_seq<Indexes...> ) BOOST_NOEXCEPT {
        return ::ddboost::typeindex::detail::cstring<s<Indexes>()...>::data_;
    }
    ...
    #else
        ddboost::typeindex::detail::failed_to_get_function_name<T>();
    #endif

        ddboost::typeindex::detail::assert_compile_time_legths<
            (size > ddboost::typeindex::detail::ctti_skip_size_at_begin + ddboost::typeindex::detail::ctti_skip_size_at_end + sizeof("const *") - 1)
        >();
        static_assert(!ddboost::typeindex::detail::ctti_skip_more_at_runtime, "Skipping for GCC in C++14 mode is unsupported");

        typedef typename ddboost::typeindex::detail::make_index_seq_impl<
            ddboost::typeindex::detail::ctti_skip_size_at_begin,
            size - sizeof("const *") + 1 - ddboost::typeindex::detail::ctti_skip_size_at_begin
        >::type idx_seq;
        return impl(idx_seq());
    }
#else
    ...
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
        return ddboost::typeindex::detail::skip_begining< sizeof(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE) >(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE);
    #elif defined(__FUNCSIG__)
        return ddboost::typeindex::detail::skip_begining< sizeof(__FUNCSIG__) >(__FUNCSIG__);
    #elif defined(__PRETTY_FUNCTION__) \
                || defined(__GNUC__) \
                || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) \
                || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) \
                ...
                || defined(__DMC__) \
                || defined(__clang__)
        return ddboost::typeindex::detail::skip_begining< sizeof(__PRETTY_FUNCTION__) >(__PRETTY_FUNCTION__);
    #else
        ddboost::typeindex::detail::failed_to_get_function_name<T>();
        return "";
    #endif
