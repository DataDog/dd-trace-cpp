...
/// \brief Contains ddboost::typeindex::ctti_type_index class that is constexpr if C++14 constexpr is supported by compiler.
///
/// ddboost::typeindex::ctti_type_index class can be used as a drop-in replacement
/// for std::type_index.
///
...
inline const detail::ctti_data& ctti_construct() BOOST_NOEXCEPT {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(ddboost::detail::ctti<T>::n());
}
...
public:
    ...
    BOOST_CXX14_CONSTEXPR inline ctti_type_index() BOOST_NOEXCEPT
        : data_(ddboost::detail::ctti<void>::n())
    {}
...
BOOST_CXX14_CONSTEXPR inline bool ctti_type_index::equal(const ctti_type_index& rhs) const BOOST_NOEXCEPT {
    ...
    return /*left == right ||*/ !ddboost::typeindex::detail::constexpr_strcmp(left, right);
}

BOOST_CXX14_CONSTEXPR inline bool ctti_type_index::before(const ctti_type_index& rhs) const BOOST_NOEXCEPT {
    const char* const left = raw_name();
    const char* const right = rhs.raw_name();
    return /*left != right &&*/ ddboost::typeindex::detail::constexpr_strcmp(left, right) < 0;
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id() BOOST_NOEXCEPT {
    ...
    typedef BOOST_DEDUCED_TYPENAME ddboost::remove_cv<no_ref_t>::type no_cvr_t;
    return ctti_type_index(ddboost::detail::ctti<no_cvr_t>::n());
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    return ctti_type_index(ddboost::detail::ctti<T>::n());
}
...
inline std::size_t ctti_type_index::hash_code() const BOOST_NOEXCEPT {
    return ddboost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}
