...
/// \brief Contains ddboost::typeindex::stl_type_index class.
///
/// ddboost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined ddboost::typeindex::ctti is usually used instead of ddboost::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "ddboost::typeindex::detail::cvr_saver<";
    ...
    const ddboost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        ddboost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return ddboost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME ddboost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME ddboost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME ddboost::conditional<
            ddboost::is_signed<no_cvr_prefinal_t>::value,
            ddboost::make_signed<no_cvr_prefinal_t>,
            ddboost::type_identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME ddboost::conditional<
        ddboost::is_reference<T>::value ||  ddboost::is_const<T>::value || ddboost::is_volatile<T>::value,
        detail::cvr_saver<T>,
        T
    >::type type;
