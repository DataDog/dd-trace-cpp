...
{
    return ::ddboost::forward<Visitable>(visitable).apply_visitor(visitor);
}
#else
template <typename Visitor, typename Visitable>
inline typename Visitor::result_type
...
struct result_multideduce1 {
    ...
    typedef typename ddboost::mpl::begin<types>::type begin_it;
    typedef typename ddboost::mpl::advance<
        begin_it, ddboost::mpl::int_<ddboost::mpl::size<types>::type::value - 1>
    >::type                                         last_it;

    template <class It, class Dummy = void> // avoid explicit specialization in class scope
    struct deduce_impl {
        typedef typename ddboost::mpl::next<It>::type next_t;
        typedef typename ddboost::mpl::deref<It>::type value_t;
        typedef decltype(true ? ddboost::declval< Visitor& >()( ddboost::declval< copy_cv_ref_t< value_t, Variant > >() )
                              : ddboost::declval< typename deduce_impl<next_t>::type >()) type;
    };

    template <class Dummy>
    struct deduce_impl<last_it, Dummy> {
        typedef typename ddboost::mpl::deref<last_it>::type value_t;
        typedef decltype(ddboost::declval< Visitor& >()( ddboost::declval< copy_cv_ref_t< value_t, Variant > >() )) type;
    };
...
struct result_wrapper1
{
    ...
    explicit result_wrapper1(Visitor&& visitor) BOOST_NOEXCEPT
        : visitor_(::ddboost::forward<Visitor>(visitor))
    {}

    template <class T>
    result_type operator()(T&& val) const {
        return visitor_(::ddboost::forward<T>(val));
    }
};
...
inline decltype(auto) apply_visitor(Visitor&& visitor, Visitable&& visitable,
    typename ddboost::disable_if<
        ddboost::detail::variant::has_result_type<Visitor>,
        bool
    >::type = true)
{
    ddboost::detail::variant::result_wrapper1<Visitor, Visitable> cpp14_vis(::ddboost::forward<Visitor>(visitor));
    return ::ddboost::forward<Visitable>(visitable).apply_visitor(cpp14_vis);
}
