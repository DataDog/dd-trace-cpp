...
#include <boost/move/utility_core.hpp> //for ddboost::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace ddboost_move_member_swap {
...
namespace ddboost_move_adl_swap{
...
template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y, typename ddboost::move_detail::enable_if_c<!ddboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename ddboost::move_detail::enable_if< and_op_not_impl<ddboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , ddboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::ddboost::move(x)); x = ::ddboost::move(y); y = ::ddboost::move(t);  }

template<class T>
BOOST_MOVE_FORCEINLINE void swap_proxy(T& x, T& y
               , typename ddboost::move_detail::enable_if< and_op_impl< ddboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , ddboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::ddboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::ddboost::move(x)); x = ::ddboost::move(y); y = ::ddboost::move(t);</code>.
template<class T>
BOOST_MOVE_FORCEINLINE void adl_move_swap(T& x, T& y)
{
   ::ddboost_move_adl_swap::swap_proxy(x, y);
}

//! Exchanges elements between range [first1, last1) and another range starting at first2
//! using ddboost::adl_move_swap.
//! 
//! Parameters:
...
{
    while (first1 != last1) {
      ::ddboost::adl_move_swap(*first1, *first2);
      ++first1;
      ++first2;
    }
   return first2;
...
{
   while (first1 != last1) {
      ::ddboost::adl_move_swap(*(--last1), *(--last2));
   }
   return last2;
}
