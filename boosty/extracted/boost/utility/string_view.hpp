...
// #endif
      ...
      BOOST_CONSTEXPR basic_string_view(const charT* str, size_type len)
        ...
        BOOST_CONSTEXPR const_reference at(size_type pos) const {
            return pos >= len_ ? BOOST_THROW_EXCEPTION(std::out_of_range("ddboost::string_view::at")), ptr_[0] : ptr_[pos];
            }
...
// "sufficient additional overloads of comparison functions"
    ...
    operator<<(std::basic_ostream<charT, traits>& os,
      const basic_string_view<charT,traits>& str) {
        return ddboost::io::ostream_put(os, str.data(), str.size());
        }
    ...
    std::size_t hash_value(basic_string_view<charT, traits> s) {
        return ddboost::hash_range(s.begin(), s.end());
        }
}
...
namespace std {
    // Hashing
    template<> struct hash<ddboost::string_view>;
    template<> struct hash<ddboost::u16string_view>;
    template<> struct hash<ddboost::u32string_view>;
    template<> struct hash<ddboost::wstring_view>;
}
#endif
