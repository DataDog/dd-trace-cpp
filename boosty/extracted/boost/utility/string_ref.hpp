...
// #endif
        ...
        const charT& at(size_type pos) const {
            if ( pos >= len_ )
                BOOST_THROW_EXCEPTION( std::out_of_range ( "ddboost::string_ref::at" ) );
            return ptr_[pos];
            }
...
namespace std {
    // Hashing
    template<> struct hash<ddboost::string_ref>;
    template<> struct hash<ddboost::u16string_ref>;
    template<> struct hash<ddboost::u32string_ref>;
    template<> struct hash<ddboost::wstring_ref>;
}
#endif
