...
private:
    ...
    ddboost::uint_least32_t line_;
    ddboost::uint_least32_t column_;
...
public:
    ...
    BOOST_CONSTEXPR source_location( char const * file, ddboost::uint_least32_t ln, char const * function, ddboost::uint_least32_t col = 0 ) BOOST_NOEXCEPT: file_( file ), function_( function ), line_( ln ), column_( col )
    {
    }
    ...
    BOOST_CONSTEXPR ddboost::uint_least32_t line() const BOOST_NOEXCEPT
    {
        return line_;
    }

    BOOST_CONSTEXPR ddboost::uint_least32_t column() const BOOST_NOEXCEPT
    {
        return column_;
    }
...
# define BOOST_CURRENT_LOCATION ::ddboost::source_location()
...
# define BOOST_CURRENT_LOCATION ::ddboost::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCTION(), __builtin_COLUMN())
...
# define BOOST_CURRENT_LOCATION ::ddboost::source_location(__FILE__, BOOST_CURRENT_LOCATION_IMPL_1(__LINE__), "")

#elif defined(__cpp_lib_source_location) && __cpp_lib_source_location >= 201907L

# define BOOST_CURRENT_LOCATION ::ddboost::source_location(::std::source_location::current())
...
# define BOOST_CURRENT_LOCATION ::ddboost::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCTION())
...
# define BOOST_CURRENT_LOCATION ::ddboost::source_location(__FILE__, __LINE__, __PRETTY_FUNCTION__)

#else

// __func__ macros aren't allowed outside functions, but BOOST_CURRENT_LOCATION is
# define BOOST_CURRENT_LOCATION ::ddboost::source_location(__FILE__, __LINE__, "")
