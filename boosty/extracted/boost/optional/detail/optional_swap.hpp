...
# define BOOST_OPTIONAL_DETAIL_MOVE(EXPR_) ddboost::move(EXPR_)
#else
...
struct swap_selector<false>
{
    template <class T>
    static void optional_swap ( optional<T>& x, optional<T>& y ) 
    //BOOST_NOEXCEPT_IF(::ddboost::is_nothrow_move_constructible<T>::value && BOOST_NOEXCEPT_EXPR(ddboost::swap(*x, *y)))
    {
        if (x)
        {
            if (y)
...
template<class T>
struct optional_swap_should_use_default_constructor : ddboost::false_type {} ;
