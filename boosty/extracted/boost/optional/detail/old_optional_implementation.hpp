...
#else
  static rval_reference_type move(reference_type r) { return ddboost::move(r); }
#endif
#endif
...
void prevent_binding_rvalue_ref_to_optional_lvalue_ref()
{
  ...
  BOOST_STATIC_ASSERT_MSG(
    !ddboost::is_lvalue_reference<To>::value || !ddboost::is_rvalue_reference<From>::value, 
...
class optional_base : public optional_tag
{
...
#endif
    ::ddboost::detail::make_reference_content<T>::type internal_type ;
  ...
  protected:
    ...
    void assign ( optional_base&& rhs )
    {
      ...
      {
        if ( rhs.is_initialized() )
             assign_value(ddboost::move(rhs.get_impl()), is_reference_predicate() );
        else destroy();
      }
      else
      {
    ...
    {
      if (is_initialized())
           assign_value( ddboost::move(val), is_reference_predicate() );
  ...
  protected :
    ...
    void emplace_assign ( Args&&... args )
     {
       destroy();
       ::new (m_storage.address()) internal_type( ddboost::forward<Args>(args)... );
       m_initialized = true ;
     }
    ...
    void emplace_assign ( Arg&& arg )
     {
       destroy();
       ::new (m_storage.address()) internal_type( ddboost::forward<Arg>(arg) );
       m_initialized = true ;
     }
    ...
    {
      new (m_storage.address()) internal_type(ddboost::forward<Expr>(expr)) ;
      m_initialized = true ;
    }
    ...
    {
      assign_value(ddboost::forward<Expr>(expr), is_reference_predicate());
    }
#else
    // Constructs using any expression implicitly convertible to the single argument
    // of a one-argument T constructor.
...
class optional : public optional_detail::optional_base<T>
{
  ...
  public :
    ...
    // Can throw if T::T(T &&) does
    optional ( rval_reference_type val ) : base( ddboost::forward<T>(val) ) 
  ...
  explicit optional ( Expr&& expr, 
                      BOOST_DEDUCED_TYPENAME ddboost::disable_if_c<
                        (ddboost::is_base_of<optional_detail::optional_tag, BOOST_DEDUCED_TYPENAME ddboost::decay<Expr>::type>::value) || 
                        ddboost::is_same<BOOST_DEDUCED_TYPENAME ddboost::decay<Expr>::type, none_t>::value, bool >::type = true 
  ) 
   ...
   // No-throw (assuming T::~T() doesn't)
    ...
    BOOST_DEDUCED_TYPENAME ddboost::disable_if_c<
      ddboost::is_base_of<optional_detail::optional_tag, BOOST_DEDUCED_TYPENAME ddboost::decay<Expr>::type>::value || 
        ddboost::is_same<BOOST_DEDUCED_TYPENAME ddboost::decay<Expr>::type, none_t>::value,
      optional&
    >::type 
    operator= ( Expr&& expr )
      {
