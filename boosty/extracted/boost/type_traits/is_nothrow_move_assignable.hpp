...
{
   BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_nothrow_move_assignable must be complete types");
};
...
struct false_or_cpp11_noexcept_move_assignable: public ::ddboost::false_type {};

template <class T>
struct false_or_cpp11_noexcept_move_assignable <
        T,
        typename ::ddboost::enable_if_<sizeof(T) && BOOST_NOEXCEPT_EXPR(::ddboost::declval<T&>() = ::ddboost::declval<T>())>::type
    > : public ::ddboost::integral_constant<bool, BOOST_NOEXCEPT_EXPR(::ddboost::declval<T&>() = ::ddboost::declval<T>())>
{};

}

template <class T>
struct is_nothrow_move_assignable : public integral_constant<bool, ::ddboost::detail::false_or_cpp11_noexcept_move_assignable<T>::value>
{
   BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_nothrow_move_assignable must be complete types");
};

template <class T> struct is_nothrow_move_assignable<T const> : public ::ddboost::false_type {};
template <class T> struct is_nothrow_move_assignable<T const volatile> : public ::ddboost::false_type{};
template <class T> struct is_nothrow_move_assignable<T volatile> : public ::ddboost::false_type{};
template <class T> struct is_nothrow_move_assignable<T&> : public ::ddboost::false_type{};
...
struct is_nothrow_move_assignable : public integral_constant<bool,
   (::ddboost::has_trivial_move_assign<T>::value || ::ddboost::has_nothrow_assign<T>::value) &&  ! ::ddboost::is_array<T>::value>
{
   BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_nothrow_move_assignable must be complete types");
};
