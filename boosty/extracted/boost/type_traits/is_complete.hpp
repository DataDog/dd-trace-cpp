...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::ddboost::is_function<typename ddboost::remove_reference<T>::type>::value || (sizeof(ddboost::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(ddboost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : ddboost::integral_constant<bool, ::ddboost::is_function<typename ddboost::remove_reference<T>::type>::value || ::ddboost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : ddboost::is_complete<T> {};
   
#else

      template <class T> struct is_complete
         : public ddboost::integral_constant<bool, true> {};
