...
// https://connect.microsoft.com/VisualStudio/feedback/details/800328/std-is-copy-constructible-is-broken
    ...
    typedef ddboost::type_traits::yes_type yes_type;
    typedef ddboost::type_traits::no_type  no_type;

    template <class U>
    static decltype(::ddboost::declval<U&>() = ::ddboost::declval<const U&>(), yes_type() ) test(int);
...
#else
    static BOOST_DEDUCED_TYPENAME ddboost::add_reference<T>::type produce();

    template <class T1>
    static ddboost::type_traits::no_type test(T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
    ...
    BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(produce())) == sizeof(ddboost::type_traits::yes_type)
    ));
   #endif
};
...
struct is_copy_assignable_impl {
    ...
    //types derived from ddboost::noncopyable and types defined as BOOST_MOVEABLE_BUT_NOT_COPYABLE
    typedef BOOST_DEDUCED_TYPENAME ddboost::remove_reference<T>::type unreferenced_t;
    BOOST_STATIC_CONSTANT(bool, value = (
        ddboost::detail::is_copy_assignable_impl2<
            ddboost::is_noncopyable<T>::value
            || ddboost::is_const<unreferenced_t>::value || ddboost::is_array<unreferenced_t>::value
            ,T
        >::value
    ));
    #else
    BOOST_STATIC_CONSTANT(bool, value = (
        ddboost::detail::is_copy_assignable_impl2<
            ddboost::is_noncopyable<T>::value,T
        >::value
    ));
    #endif
};
...
template <class T> struct is_copy_assignable : public integral_constant<bool, ::ddboost::detail::is_copy_assignable_impl<T>::value>{};
