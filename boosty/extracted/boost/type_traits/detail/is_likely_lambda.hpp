...
//
namespace ddboost{ namespace type_traits_detail{
...
{
  template<typename Q> static ddboost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static ddboost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename ddboost::enable_if_<has_one_operator_call<T>::value>::type> :
     ddboost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace ddboost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public ddboost::integral_constant<bool, false> {};
}}
