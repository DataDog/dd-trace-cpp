...
private:
    ...
    typedef typename ddboost::remove_cv<T>::type T2;
    typedef typename ddboost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (ddboost::is_same<T2, U2>::value)
        || ddboost::is_void<T2>::value
        || ddboost::is_void<U2>::value
        || (ddboost::is_base_of<T2, U2>::value)
        || (ddboost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename ddboost::conditional<

        ddboost::is_same<T2, U2>::value || ddboost::is_void<T2>::value || ddboost::is_base_of<T2, U2>::value,
        typename ddboost::copy_cv<T, U>::type,
        typename ddboost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename ddboost::copy_cv<typename ddboost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using ddboost::conditional in
