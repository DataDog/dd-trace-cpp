...
namespace ddboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(ddboost::declval<TheArgs>()...))>
         static ddboost::type_traits::yes_type test(int);
         template<typename, typename...>
         static ddboost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(ddboost::declval<Arg>()))>
         static ddboost::type_traits::yes_type test1(int);
         template<typename, typename>
         static ddboost::type_traits::no_type test1(...);

         template <typename T>
         static ddboost::type_traits::yes_type ref_test(T);
         template <typename T>
         static ddboost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(ddboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(::ddboost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(ddboost::detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(ddboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(::ddboost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(ddboost::declval<Arg>())) == sizeof(ddboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(ddboost::declval<Arg>())) == sizeof(ddboost::type_traits::yes_type)>{};
