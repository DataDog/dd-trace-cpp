...
namespace ddboost{
   ...
   namespace detail{
      ...
      {
          static const bool value = sizeof(is_default_constructible_imp::test<T>(0)) == sizeof(ddboost::type_traits::yes_type);
      };
      template<class T> 
...
#if BOOST_WORKAROUND(BOOST_GCC_VERSION, < 40700)
   template <class T> struct is_default_constructible : public integral_constant<bool, detail::is_default_constructible_abstract_filter<T, ddboost::is_abstract<T>::value>::value>
   {
      BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_default_constructible must be complete types");
   };
#else
   template <class T> struct is_default_constructible : public integral_constant<bool, sizeof(ddboost::detail::is_default_constructible_imp::test<T>(0)) == sizeof(ddboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_default_constructible must be complete types");
   };
#endif
