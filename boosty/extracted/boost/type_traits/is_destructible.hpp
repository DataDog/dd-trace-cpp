...
namespace ddboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(ddboost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(ddboost::detail::is_destructible_imp::test<T>(0)) == sizeof(ddboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_destructible must be complete types");
   };
