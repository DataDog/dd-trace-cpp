...
template <class T> struct is_copy_constructible : public ddboost::is_constructible<T, const T&>{};
...
// ddboost::non_copyable as a base class as a special case:
//
...
namespace ddboost {
   ...
   namespace detail
   {
      ...
      template <class T, bool b> struct is_copy_constructible_imp : public ddboost::is_constructible<T, const T&>{};
...
namespace ddboost {
   ...
   namespace detail{
      ...
      struct is_copy_constructible_impl2 {
         ...
         template <class T1>
         static ddboost::type_traits::yes_type test(const T1&, ddboost::mpl::int_<sizeof(T1(ddboost::declval<const T1&>()))>* = 0);
#else
         template <class T1>
         static ddboost::type_traits::yes_type test(const T1&, decltype(T1(ddboost::declval<const T1&>()))* = 0);
#endif

         static ddboost::type_traits::no_type test(...);
#else
         template <class T1>
         static ddboost::type_traits::no_type test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
         static ddboost::type_traits::yes_type test(...);
#endif
         ...
         //      struct T: private ddboost::noncopyable {
         //          ...
         //      private:
         //          T(const T &);
         //          ...
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
            ddboost::declval<BOOST_DEDUCED_TYPENAME ddboost::add_reference<T const>::type>()
            )) == sizeof(ddboost::type_traits::yes_type)
            &&
            !ddboost::is_rvalue_reference<T>::value
            && !ddboost::is_array<T>::value
            ));
      };
      ...
      struct is_copy_constructible_impl {
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            ddboost::detail::is_copy_constructible_impl2<
            ddboost::is_noncopyable<T>::value,
            T
            >::value
            ));
      };
   ...
   template <class T> struct is_copy_constructible : public integral_constant<bool, ::ddboost::detail::is_copy_constructible_impl<T>::value>{};
   template <> struct is_copy_constructible<void> : public false_type{};
