...
namespace ddboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(ddboost::declval<T>() = ddboost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(ddboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(ddboost::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
