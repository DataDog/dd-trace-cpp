...
namespace ddboost
{
    namespace hash_detail
    {
        ...
        template <> struct basic_numbers<bool> :
            ddboost::hash_detail::enable_hash_value {};
        ...
        template <> struct long_numbers<ddboost::long_long_type> :
            ddboost::hash_detail::enable_hash_value {};
        template <> struct ulong_numbers<ddboost::ulong_long_type> :
            ddboost::hash_detail::enable_hash_value {};
#endif

#if defined(BOOST_HAS_INT128)
        template <> struct long_numbers2<ddboost::int128_type> :
            ddboost::hash_detail::enable_hash_value {};
        template <> struct ulong_numbers2<ddboost::uint128_type> :
            ddboost::hash_detail::enable_hash_value {};
#endif
    ...
    template <typename T>
    typename ddboost::hash_detail::basic_numbers<T>::type hash_value(T);
    template <typename T>
    typename ddboost::hash_detail::long_numbers<T>::type hash_value(T);
    template <typename T>
    typename ddboost::hash_detail::ulong_numbers<T>::type hash_value(T);

    template <typename T>
    typename ddboost::enable_if<ddboost::is_enum<T>, std::size_t>::type
        hash_value(T);
    ...
    template <typename T>
    typename ddboost::hash_detail::float_numbers<T>::type hash_value(T);
    ...
    namespace hash_detail
    {
        ...
        {
            inline static ddboost::uint32_t fn(ddboost::uint32_t h1, ddboost::uint32_t k1)
            {
                const ddboost::uint32_t c1 = 0xcc9e2d51;
                const ddboost::uint32_t c2 = 0x1b873593;
        ...
        {
            inline static ddboost::uint64_t fn(ddboost::uint64_t h, ddboost::uint64_t k)
            {
                const ddboost::uint64_t m = (ddboost::uint64_t(0xc6a4a793) << 32) + 0x5bd1e995;
                const int r = 47;
    ...
    template <typename T>
    typename ddboost::hash_detail::basic_numbers<T>::type hash_value(T v)
    {
        return static_cast<std::size_t>(v);
    }

    template <typename T>
    typename ddboost::hash_detail::long_numbers<T>::type hash_value(T v)
    {
        return hash_detail::hash_value_signed(v);
    }

    template <typename T>
    typename ddboost::hash_detail::ulong_numbers<T>::type hash_value(T v)
    {
...
#pragma warning(disable:4267) // 'argument' : conversion from 'size_t' to
    ...
    {
        ddboost::hash<T> hasher;
        seed = ddboost::hash_detail::hash_combine_impl<sizeof(std::size_t) * CHAR_BIT>::fn(seed, hasher(v));
    }
...
#pragma warning(pop)
    ...
    template <typename T>
    typename ddboost::hash_detail::float_numbers<T>::type hash_value(T v)
    {
        return ddboost::hash_detail::float_hash_value(v);
    }
    ...
    // ddboost::hash is defined later in extensions.hpp if
...
#define BOOST_HASH_SPECIALIZE(type) \
    template <> struct hash<type> \
         : public ddboost::hash_detail::hash_base<type> \
    { \
        std::size_t operator()(type v) const \
        { \
            return ddboost::hash_value(v); \
        } \
    };
...
#if !defined(BOOST_NO_LONG_LONG)
    BOOST_HASH_SPECIALIZE(ddboost::long_long_type)
    BOOST_HASH_SPECIALIZE(ddboost::ulong_long_type)
#endif

#if defined(BOOST_HAS_INT128)
    BOOST_HASH_SPECIALIZE(ddboost::int128_type)
    BOOST_HASH_SPECIALIZE(ddboost::uint128_type)
#endif
...
// Specializing ddboost::hash for pointers.

#if !defined(BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION)

    template <class T>
    struct hash<T*>
        : public ddboost::hash_detail::hash_base<T*>
    {
    ...
    // ddboost::hash for all remaining types. But hash_impl is only defined
    ...
    template <class T> struct hash
        : public ddboost::hash_detail::hash_impl<ddboost::is_pointer<T>::value>
