...
// `EventScheduler` is an interface that allows a specified function-like object
// to be invoked at regular intervals.
//
// `DatadogAgent` uses an `EventScheduler` to periodically send batches of
// traces to the Datadog Agent, and to poll it for Remote Configuration updates.
//
...
class EventScheduler {
 public:
  ...
  // Arrange for the specified `callback` to be invoked repeatedly, with the
  // specified `interval` elapsing between invocations. The first invocation is
  // after an initial `interval`. Return a function-like object that can be
  // invoked without arguments to prevent subsequent invocations of `callback`.
