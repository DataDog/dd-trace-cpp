...
TEST_SPAN("injection") {
  ...
  SECTION("trace source is propagated") {
    ...
    span.set_tag(tags::internal::trace_source,
                 tags::internal::source::database_monitoring);
    ...
    CHECK(found->second == tags::internal::source::database_monitoring);
  }
}
...
TEST_SPAN("injection behaviour when apm tracing is disabled") {
  ...
  SECTION("tracing context is not propagated") {
    auto span = tracer.create_span();
  ...
  SECTION("tracing context is propagated when `_dd.p.ts` is set") {
    auto span = tracer.create_span();
    span.set_tag(tags::internal::trace_source, tags::internal::source::appsec);
    ...
    const auto& headers = writer.items;
    CHECK(headers.count("x-datadog-trace-id") == 1);
    CHECK(headers.count("x-datadog-parent-id") == 1);
  }
}
