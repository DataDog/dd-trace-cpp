...
TELEMETRY_IMPLEMENTATION_TEST("Tracer telemetry lifecycle") {
  ...
  SECTION("ctor send app-started message") {
    SECTION("Without a defined integration") {
      auto telemetry = Telemetry::create(*finalize_config(),
                          tracer_signature,
                          logger,
                          client,
                          scheduler,
    ...
    SECTION("With an integration") {
      ...
      auto telemetry2 = Telemetry::create(*finalize_config(cfg),
                           tracer_signature,
                           logger,
                           client,
                           scheduler,
    ...
    SECTION("With installation signature") {
      ...
      auto telemetry4 = Telemetry::create(*finalize_config(),
                           tracer_signature,
                           logger,
                           client,
                           scheduler,
    ...
    SECTION("With configuration") {
      ...
      auto telemetry3 = Telemetry::create(*finalize_config(cfg),
                           tracer_signature,
                           logger,
                           client,
                           scheduler,
...
TELEMETRY_IMPLEMENTATION_TEST("Tracer telemetry API") {
  ...
  SECTION("metrics reporting") {
    ...
    SECTION("dtor sends metrics and distributions") {
      ...
      const Counter my_counter{"my_counter", "counter-test", true};
      {
        auto tmp_telemetry = Telemetry::create(*finalize_config(),
                                tracer_signature,
                                logger,
                                client,
                                scheduler,
...
TELEMETRY_IMPLEMENTATION_TEST("Tracer telemetry configuration") {
  ...
  SECTION("disabling metrics reporting do not collect metrics") {
    ...
    auto telemetry = Telemetry::create(*final_cfg, tracer_signature, logger, client, scheduler,
                        *url);
