...
class TracerConfig {
 public:
  ...
  TracerConfig& set_datadog_agent_url(std::string url);
  ...
  TracerConfig& enable_traces(bool report_traces);
  ...
  TracerConfig& enable_telemetry(bool report_telemetry);
  ...
  TracerConfig& set_injection_styles(std::vector<PropagationStyle> injeciton_styles);
  ...
  // environment variables.
  TracerConfig& set_extraction_styles(std::vector<PropagationStyle> extraction_styles);
  ...
  TracerConfig& set_trace_sampler(TraceSamplerConfig trace_sampler);
  ...
  // environment variables.
  TracerConfig& set_span_sampler(SpanSamplerConfig trace_sampler);

  // TODO:
  TracerConfig& set_sampling_delegation(bool enable_sampling_deleation);

  // `report_hostname` indicates whether the tracer will include the result of
  // `gethostname` with traces sent to the collector.
  TracerConfig& enable_hostname_in_span(bool report_hostname);
  ...
  // not to be null.
  TracerConfig& set_http_client(std::shared_ptr<HTTPClient> http_client);
  ...
  TracerConfig& set_collector(std::shared_ptr<Collector> collector);
  ...
  TracerConfig& set_max_tags_header_size(std::size_t max_tags_header_size);
  ...
  // `std::cerr`.
  TracerConfig& set_logger(std::shared_ptr<Logger> logger);
  ...
  TracerConfig& set_runtime_id(RuntimeID runtime_id);

  // Set how often, in seconds, to query the Datadog Agent for remote configuration
  // updates.
  TracerConfig& set_remote_configuration_poll_interval(std::chrono::seconds poll_interval);
