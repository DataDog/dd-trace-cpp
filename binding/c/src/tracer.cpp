...
ddog_trace_conf_t* ddog_trace_tracer_conf_new() { return new dd::TracerConfig; }

void ddog_trace_tracer_conf_free(ddog_trace_conf_t* handle) {
  if (!handle) return;
  delete static_cast<dd::TracerConfig*>(handle);
}

void ddog_trace_tracer_conf_set(ddog_trace_conf_t* handle,
                                enum ddog_trace_tracer_option option,
                                const char* value) {
  if (!handle || !value) return;

  auto* cfg = static_cast<dd::TracerConfig*>(handle);

  if (option == DDOG_TRACE_OPT_SERVICE_NAME) {
    cfg->service = value;
  } else if (option == DDOG_TRACE_OPT_ENV) {
    cfg->environment = value;
  } else if (option == DDOG_TRACE_OPT_VERSION) {
    cfg->version = value;
  } else if (option == DDOG_TRACE_OPT_AGENT_URL) {
    cfg->agent.url = value;
  } else if (option == DDOG_TRACE_OPT_INTEGRATION_NAME) {
    cfg->integration_name = value;
  } else if (option == DDOG_TRACE_OPT_INTEGRATION_VERSION) {
    cfg->integration_version = value;
  }
}

ddog_trace_tracer_t* ddog_trace_tracer_new(ddog_trace_conf_t* conf_handle) {
  if (!conf_handle) return nullptr;
...
void ddog_trace_tracer_free(ddog_trace_tracer_t* tracer_handle) {
  if (!tracer_handle) return;
  delete static_cast<dd::Tracer*>(tracer_handle);
}

ddog_trace_span_t* ddog_trace_tracer_create_span(
    ddog_trace_tracer_t* tracer_handle, const char* name) {
  if (!tracer_handle || !name) return nullptr;

  auto* tracer = static_cast<dd::Tracer*>(tracer_handle);
  dd::SpanConfig opt;
  opt.name = name;

  return new dd::Span(tracer->create_span(opt));
}
...
ddog_trace_span_t* ddog_trace_tracer_extract_or_create_span(
    ...
    const char* resource) {
  if (!tracer_handle || !on_context_read || !name) return nullptr;
...
void ddog_trace_span_set_resource(ddog_trace_span_t* span_handle,
                                  const char* resource) {
  if (!span_handle || !resource) return;
  static_cast<dd::Span*>(span_handle)->set_resource_name(resource);
}

void ddog_trace_span_set_service(ddog_trace_span_t* span_handle,
                                 const char* service) {
  if (!span_handle || !service) return;
  static_cast<dd::Span*>(span_handle)->set_service_name(service);
}

void ddog_trace_span_set_tags(ddog_trace_span_t* span_handle, const char** keys,
                              const char** values, int count) {
  if (!span_handle || !keys || !values || count <= 0) return;
...
int ddog_trace_span_get_sampling_priority(ddog_trace_span_t* span_handle,
                                          int* priority) {
  if (!span_handle || !priority) return -1;
...
ddog_trace_span_t* ddog_trace_span_create_child_with_options(
