...
class HeaderReader : public dd::DictReader {
  ...
  public:
    explicit HeaderReader(const httplib::Headers& headers)
    : headers_(headers) {}
    ...
    void visit(
        const std::function<void(std::string_view key, std::string_view value)>& visitor)
        const override {
...
int main() {
    ...
    server.set_pre_request_handler([](httplib::Request& request, httplib::Response&) {
    ...
    server.set_pre_routing_handler([&](const httplib::Request& request, httplib::Response&) {
        ...
        if (dd::Error *error = maybe_span.if_error()) {
            std::cerr << "While extracting trace context from request: "
                      << *error << '\n';
    ...
    // TODO: Explain when this happens.
    server.Get("/healthcheck", [&](const httplib::Request& request, httplib::Response& response) {
        std::cerr << "Here we are in /healthcheck\n"; // TODO
        auto *context = static_cast<RequestTracingContext*>(request.user_data.get());

        // We'd prefer not to send healthcheck traces to Datadog. They're noisy.
        // So, override the sampling decision to "definitely drop," and don't
        // even bother creating a span here.
    ...
    // TODO: Explain when this happens.
    server.Get("/notes", [&](const httplib::Request& request, httplib::Response& response) {
